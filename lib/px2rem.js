...
var defaultConfig = {
  ...
  keepPxValue: '1', // ignore convert if value is 1px
...
function keepPXValue(declaration, config) {
  var pxValues = config.keepPxValue.split(',').map((s) => s.trim());
  var matched = declaration.value.match(pxRegExp);
  if (matched) {
    return pxValues.includes(parseInt(matched[0]).toString())
  }
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      for (var j = 0; j < declarations.length; j++) {
        ...
        if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
          if (keepPXValue(declaration, config)) {
            continue;
          }
