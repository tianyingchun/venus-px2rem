...
var defaultConfig = {
  ...
  keep1px: true,          // ignore convert if value is 1px
  keepComment: 'no'       // no transform value comment (default: `no`)
};

var pxRegExp = /\b(\d+(\.\d+)?)px\b/;
var px1RegExp = /\b1px\b/;
...
function ignore1pxConvert(declaration, config) {
  var matched = declaration.type === 'declaration' && pxRegExp.test(declaration.value);
  if (config.keep1px === true) {
    return matched && !px1RegExp.test(declaration.value);
  }
  return matched;
}
  ...
  function processRules(rules, noDealPx) { // FIXME: keyframes do not support `force px` comment
    for (var i = 0; i < rules.length; i++) {
      ...
      for (var j = 0; j < declarations.length; j++) {
        var declaration = declarations[j];
        // need transform: declaration && has 'px'
        if (ignore1pxConvert(declaration, config)) {
          var nextDeclaration = rule.declarations[j + 1];
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
